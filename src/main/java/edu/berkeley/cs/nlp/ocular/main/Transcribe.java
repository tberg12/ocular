...
//	public static boolean evalBatches = false;
	...
	protected static void validateOptions() {
		...
		if (!(trainFont == (outputFontPath != null))) throw new IllegalArgumentException("-trainFont is not as expected");
	}
	...
	public void run() {
		...
		if (skipAlreadyTranscribedDocs) {
			for (Iterator<Document> itr = inputDocuments.iterator(); itr.hasNext(); ) {
				...
				String docTranscriptionPath = BasicSingleDocumentEvaluatorAndOutputPrinter.makeOutputFilenameBase(doc, newInputDocPath, outputPath);
		...
		if (outputFontPath != null) {
			...
			transcriber.printTranscriptionWithEvaluation(0, 0, initialFont, initialLM, initialGSM);
		}
	}
